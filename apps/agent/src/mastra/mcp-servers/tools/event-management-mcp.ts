 ...
 * Wraps the main updateCalendarEvent tool for MCP server usage
 */
export const updateCalendarEventMCP = createTool({
  id: 'updateCalendarEvent',
  description: `Update calendar events with comprehensive field support via MCP.
...
- Duration is specified in minutes as integer`,
  ...
  execute: async (executionContext, options) => {
    ...
    console.log('MCP updateCalendarEvent:', context);

    const userJwt = getJwtFromContext({ runtimeContext: executionContext.runtimeContext });
    console.log('updateCalendarEventMCP - JWT available:', !!userJwt);
    ...
    try {
      ...
      console.error('Error updating calendar events via MCP:', error);
      return {
        success: false,
 ...
 * Wraps the main getCalendarEvents tool for MCP server usage
 */
export const getCalendarEventsMCP = createTool({
  id: 'getCalendarEvents',
  description: 'Get calendar events for a date range via MCP',
  inputSchema: z.object({
  ...
  execute: async (executionContext, options) => {
    const { context } = executionContext;
    console.log('MCP getCalendarEvents:', context);

    const userJwt = executionContext.runtimeContext?.get('jwt-token');
    console.log('getCalendarEventsMCP - JWT available:', !!userJwt);
