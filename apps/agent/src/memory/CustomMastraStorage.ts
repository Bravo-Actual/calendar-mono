...
import type { WorkflowRun, WorkflowRuns } from '@mastra/core/workflows';
import { Pool } from 'pg';
...
export class CustomMastraStorage extends MastraStorage {
  ...
  async saveThread({ thread }: { thread: StorageThreadType }): Promise<StorageThreadType> {
    console.log('🔥 CustomMastraStorage.saveThread called!', {
      threadId: thread.id,
  ...
  async saveMessages(args: any): Promise<any> {
    console.log('🔥 CustomMastraStorage.saveMessages called!', {
  ...
  async updateThread({
    ...
    await this.pool.query('UPDATE ai_memory_threads SET title = $2, metadata = $3 WHERE id = $1', [
      id,
      title,
  ...
  async getResourceById({
    ...
    try {
      const res = await this.pool.query('SELECT * FROM ai_resources WHERE resource_id = $1', [
        resourceId,
      ]);
  ...
  async getTraces(_args: StorageGetTracesArg): Promise<Trace[]> {
    return [];
  }
  async getTracesPaginated(
    _args: StorageGetTracesPaginatedArg
  ...
  async updateWorkflowResults(_args: any): Promise<any> {
    return {};
  }
  async updateWorkflowState(_args: any): Promise<any> {
    return undefined;
  }
  async getScoreById({ id }: { id: string }): Promise<ScoreRowData | null> {
    return null;
  }
  async saveScore(_score: ValidatedSaveScorePayload): Promise<{ score: ScoreRowData }> {
  ...
  async getEvals(_options: any): Promise<any> {
    return { evals: [], totalCount: 0, page: 1, perPage: 10 };
  }
  async getEvalsByAgentName(_agentName: string, _type?: 'test' | 'live'): Promise<EvalRow[]> {
    return [];
  }
  async getWorkflowRuns(_args?: any): Promise<WorkflowRuns> {
    return { runs: [], totalCount: 0 };
  }
  async getWorkflowRunById(_args: any): Promise<WorkflowRun | null> {
    return null;
  }
}
