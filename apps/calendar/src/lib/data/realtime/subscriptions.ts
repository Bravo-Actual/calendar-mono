      ...
      async (payload) => {
        ...
        if (eventType === 'DELETE') {
          ...
          queryClient.setQueryData(
            keys.calendars(uid),
            (old?: ClientCalendar[]) => old?.filter((cal) => cal.id !== o.id) ?? []
          );
        } else {
    ...
    .on(
      ...
      async (payload) => {
        ...
        if (eventType === 'DELETE') {
          ...
          queryClient.setQueryData(
            keys.categories(uid),
            (old?: ClientCategory[]) => old?.filter((cat) => cat.id !== o.id) ?? []
          );
        } else {
    ...
    .on(
      ...
      async (payload) => {
        ...
        if (eventType === 'DELETE') {
          ...
          queryClient.setQueryData(
            keys.personas(uid),
            (old?: ClientPersona[]) => old?.filter((persona) => persona.id !== o.id) ?? []
          );
        } else {
        ...
        if (eventType === 'DELETE') {
          ...
          queryClient.setQueriesData(
            ...
            (old?: ClientAnnotation[]) => {
              ...
              return [...filtered, annotation].sort(
                (a, b) => (a.start_time_ms || 0) - (b.start_time_ms || 0)
              );
            }
          );
        }
