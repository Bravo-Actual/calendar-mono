...
const _SIXTY_FPS_INTERVAL_MS = 1000 / 60;
...
export const Conversation = ({
  ...
  const animateScroll = useCallback(
    (timestamp: number) => {
      ...
      const acceleration =
        (springStiffness * distance - springDamping * state.velocity) / springMass;
  ...
  const scrollToBottom: ScrollToBottom = useCallback(
    (options) => {
      const viewport = getViewport();
      if (!viewport) return false;
    ...
    [getViewport, getTargetScrollTop, resize, stopScroll, animateScroll, escapedFromLock]
  );
