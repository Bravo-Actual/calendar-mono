...
// Create events from ranges (pure, testable)
export function createEventsFromRanges(ranges: SelectedTimeRange[], defaultTitle = "New event"): CalEvent[] {
  return ranges.map((r) => ({
    id: uid("evt"),
    title: defaultTitle,
    start: r.startAbs,
...
// Delete events by id set (pure, testable)
export function deleteEventsByIds(events: CalEvent[], ids: Set<EventId>): CalEvent[] {
...
const CalendarWeek = forwardRef<CalendarWeekHandle, CalendarWeekProps>(function CalendarWeek(
  ...
  return (
    <div className="relative w-full select-none text-sm">
      ...
      {/* Floating toolbar (single instance) */}
...
export function runCalendarWeekSelfTest() {
  ...
  // createEventsFromRanges basic test
  (function () {
    const ranges: SelectedTimeRange[] = [
      { id: "r1", dayIdx: 0, startAbs: 1000, endAbs: 2000 },
      { id: "r2", dayIdx: 1, startAbs: 3000, endAbs: 4000 },
    ];
    const evts = createEventsFromRanges(ranges, "X");
    results.create_from_ranges_len = evts.length === 2;
    results.create_from_ranges_titles = evts.every(e => e.title === "X");
    results.create_from_ranges_times = evts[0].start === 1000 && evts[0].end === 2000 && evts[1].start === 3000 && evts[1].end === 4000;
  })();

  // deleteEventsByIds test
  (function () {
    const E: CalEvent[] = [
      { id: "a", title: "A", start: 0, end: 1 },
      { id: "b", title: "B", start: 2, end: 3 },
      { id: "c", title: "C", start: 4, end: 5 },
    ];
    const left = deleteEventsByIds(E, new Set<EventId>(["a", "c"]));
    results.delete_keeps_only_b = left.length === 1 && left[0].id === "b";
  })();
