...
export function parseWeekStart(
  ...
  const base = initialISO
    ? Temporal.ZonedDateTime.from(`${initialISO}[${tz}]`)
...
export function snapMs(ms: number, step: number) {
...
export function formatTimeRangeLabel(
  ...
  endMs: number,
  tz: string,
  format: '12_hour' | '24_hour' = '12_hour'
) {
...
export function computeFreeGapsForDay(
  ...
  const dayEvents = events
    ...
    .map((e) => {
      return {
        start: Math.max(e.start_time_ms, dayStartAbs),
        end: Math.min(e.end_time_ms, dayEndAbs),
      };
    })
...
export function recommendSlotsForDay(
  ...
  const gaps = computeFreeGapsForDay(events, dayStartAbs, dayEndAbs).filter(
    (g) => g.end - g.start >= durationMs
  );
...
export async function createEventsFromRanges(
  ranges: { startAbs: number; endAbs: number }[],
