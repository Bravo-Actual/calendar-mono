...
export const navigateCalendarMCP = createTool({
  ...
  execute: async ({ context }) => {
    // Server-side MCP tool - returns navigation instructions for the frontend to execute
    ...
    if (context.dates && Array.isArray(context.dates)) {
      ...
      const validDates = context.dates.filter((date) => /^\d{4}-\d{2}-\d{2}$/.test(date)).sort();
      ...
      return {
        ...
        message: `Calendar navigated to ${validDates.length} non-consecutive dates via MCP: ${validDates.join(', ')}`,
      };
      ...
      return {
        ...
        message: `Calendar navigated to consecutive range via MCP: ${startDate} to ${endDate} (${finalDayCount} days)`,
      };
    } else {
      return {
        success: false,
        error:
          'Must provide either startDate (for consecutive mode) or dates array (for non-consecutive mode)',
      };
    }
  },
});
