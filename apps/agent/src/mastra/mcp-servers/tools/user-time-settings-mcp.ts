...
export const getUserTimeSettingsMCP = createTool({
  ...
  execute: async (executionContext) => {
    ...
    try {
      const response = await fetch(
        `${process.env.SUPABASE_URL}/rest/v1/user_profiles?select=timezone,time_format,week_start_day`,
        {
          headers: {
      ...
      return {
        success: true,
        settings,
        message: `User time settings: ${settings.timezone}, ${settings.timeFormat} format, week starts on ${weekStartDayName}`,
      };
    } catch (error) {
      return {
        success: false,
...
export const updateUserTimeSettingsMCP = createTool({
  ...
  inputSchema: z.object({
    timezone: z
      .string()
      .optional()
      .describe("IANA timezone identifier (e.g., 'America/Chicago', 'Europe/London')"),
    timeFormat: z.enum(['12_hour', '24_hour']).optional().describe('Time display format'),
    weekStartDay: z
      .enum(['0', '1', '2', '3', '4', '5', '6'])
      .optional()
      .describe('Day week starts on (0=Sunday, 1=Monday, etc.)'),
  }),
  execute: async (executionContext) => {
    const jwt = executionContext.runtimeContext?.get('jwt-token');
