...
export const highlightEventsTool = createTool({
  id: "highlightEvents",
  description: "Highlight specific events on the calendar to draw user attention. This creates yellow AI highlights separate from user selections. When the user refers to events they have already selected, use the exact event IDs from the selectedEvents array in the calendar context.",
  inputSchema: z.object({
    eventIds: z.array(z.string()).describe("Array of event IDs to highlight with yellow AI highlight. When user refers to their selected events, use the exact IDs from selectedEvents in calendar context."),
