...
const getStatusBadge = (status: ToolUIPart['state']) => {
  ...
  return (
    <Badge className="rounded-full text-[10px] px-1.5 py-0.5 gap-1" variant="secondary">
      {icons[status]}
      {labels[status]}
    </Badge>
  );
...
export const ToolInput = ({ className, input, ...props }: ToolInputProps) => (
  <div className={cn('space-y-2 overflow-hidden p-2 min-w-0', className)} {...props}>
...
export const ToolOutput = ({ className, output, errorText, ...props }: ToolOutputProps) => {
  ...
  if (typeof output === 'object' && output !== null) {
    displayOutput = <CodeBlock code={JSON.stringify(output, null, 2)} language="json" />;
  } else if (typeof output === 'string') {
    displayOutput = <div className="whitespace-pre-wrap">{output}</div>;
  }

  return (
    <div className={cn('space-y-2 p-2 min-w-0', className)} {...props}>
      <h4 className="font-medium text-muted-foreground text-[10px] uppercase tracking-wide">
      ...
      >
        ...
        {displayOutput && <div className="p-2 min-w-0">{displayOutput}</div>}
      </div>
    </div>
  );
};
